...
def create_subtitle_clip(text, duration, start_time, video_width, video_height):
    ...
    # Convert to numpy for ImageClip
    np_img = np.array(img)
    return ImageClip(np_img).set_duration(duration).set_start(start_time).set_position(("center", video_height * 0.75))
